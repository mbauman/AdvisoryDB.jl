#!/usr/bin/env julia
#
# USAGE: fetch_github_advisories OUT_DIRECTORY [optional search mechanism]

using AdvisoryDB: GitHub, JSON3

try
    println("Starting GitHub Security Advisory fetcher...")
    if isempty(ARGS)
        path = "/tmp/ghsa"
    end

    all_advisories = if length(ARGS) > 1 && !isempty(ARGS[2])
        # Fetch all advisories for an org or single repo (for manual triggering)
        if contains(ARGS[2], "/")
            GitHub.fetch_repo_advisories(ARGS[2])
        else
            GitHub.fetch_org_advisories(ARGS[2])
        end
    else
        GitHub.fetch_advisories()
    end
    n = length(all_advisories)
    println("Found $n total GHSA$(n==1 ? "" : "s")")
    advisories = GitHub.filter_julia_advisories(all_advisories)

    if isempty(advisories)
        println("No Julia ecosystem advisories found")
        return
    end

    n = length(advisories)
    println("Found $n Julia-related GHSA$(n==1 ? "" : "s")")

    mkpath(path)
    for advisory in advisories
        JSON3.write(joinpath(p, string(advisory.ghsa_id, ".json")))
    end

catch ex
    # GitHub.create_issue
    println("Error: $e")
    exit(1)
end
